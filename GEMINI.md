...
| Phase 15 Tier-1 Enterprise Production Remediation & Security Hardening | ✅ Completed (24-point remediation: sealed payment landing URLs, tracking token spoofing guard, caller-to-order ownership, atomic flash deal checkout deduction, 100-query N+1 elimination, composite DB indexes, Heroku v93 deploy, 14/14 live tests pass) | Done |
...
40. **Async Non-Blocking I/O & Polling Concurrency Invariant:** All post-order side effects (Gmail SMTP email notifications, FCM push broadcasts) in `backend/orders/views.py` MUST be dispatched asynchronously via `ThreadPoolExecutor(max_workers=4)` with defensive exception logging to prevent blocking Gunicorn worker threads. `OrderTrackView` MUST exempt anonymous tracking requests from `AnonRateThrottle` (`throttle_classes = []`) and use `.filter(query).order_by('-id').first()` to eliminate rate limits and multi-match crashes. Web Admin (`AdminContext.tsx`) polling MUST NOT exceed 15s interval, must eliminate duplicate fetch ticks, and must suspend polling when document visibility is hidden (`document.visibilityState === 'hidden'`). Mobile clients MUST NOT run duplicate polling hooks or auto-polling loops on static menu screens.
41. **Tier-1 Payment Integrity, Object Authorization & Tracking Token Invariant:** Public payment success landing endpoints (`StripeSuccessLandingView`, `PayFastSuccessLandingView`) MUST NEVER mutate payment or order status upon HTTP GET; transitions to `completed`/`received` MUST occur strictly via cryptographically signed webhooks. Payment confirmation (`ConfirmCODPaymentView`, `ConfirmStripePaymentView`, `CreatePayFastPaymentView`) and order review/detail endpoints MUST enforce object-level authorization (`order.user == request.user` or matching `tracking_token`). String token equality MUST require a truthy `order.tracking_token` before comparison (`order.tracking_token and token and str(order.tracking_token).strip() == str(token).strip()`) to prevent `'None' == 'None'` spoofing. Order resolution in payment handlers MUST support polymorphic resolution across integer `pk` and alphanumeric `display_order_id` via `get_order_by_identifier`. `OrderAssignRiderView` MUST verify caller-to-order management permissions (`order.branch == managed_branch` or `order.restaurant == managed_restaurant`) for non-superusers.
